...
void Renderer::Render(Snake const snake, EnemySnake const enemy, SDL_Point const &food)
{
  SDL_Rect block;
  ...
  {
    // Render enemy's body
    SDL_SetRenderDrawColor(sdl_renderer, 0x98, 0xFB, 0x98, 0xFF);
...
{
  std::string title{"Score: " + std::to_string(score) + "\tHigh Score: " + std::to_string(high_score) + "\tFPS: " + std::to_string(fps)};
